...
<div class="container-fluid">
    <form action="" id="journal-form">
		...
		<input type="hidden" name="journal_type" value="dv">
		<input type="hidden" name="new" id="new" value="0">
		<input type="hidden" name="debit_count" id="debit_count" value="<?= isset($debit_count) ? $debit_count :'' ?>">
		<input type="hidden" name="credit_count" id="credit_count" value="<?= isset($credit_count) ? $credit_count :'' ?>">
        <div class="row">
			...
			<div class="col-md-2 form-group">
				Source of Fund<br>
				<select id="source_fund" name="source_fund" class="from-control form-control-sm" required>
		...
		<div class="row">
			<div class="col-md-6 form-group">
                Component No<br><select id="component_number" name="component_number" class="from-control form-control-sm select2" data-width="auto" required>				
                    <option value="" disabled selected></option>
                    <?php 
                    $components = $conn->query("SELECT * FROM `tbl_component`");
                    while($comp = $components->fetch_assoc()):
                    ?>
                    <option value="<?= $comp['id'] ?>" <?= isset($component_number) && $component_number == $comp['id'] ? "selected" : "" ?>><?= $comp['com_name'] ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
			<div class="col-md-6 form-group">
                Category No<br><select id="category_number" name="category_number" class="from-control form-control-sm select2" data-width="auto" required>
                    <option value="" disabled selected></option>
                    <?php 
                    $components = $conn->query("SELECT * FROM `tbl_category`");
                    while($comp = $components->fetch_assoc()):
                    ?>
                    <option value="<?= $comp['id'] ?>" <?= isset($category_number) && $category_number == $comp['cat_id'] ? "selected" : "" ?>><?= $comp['cat_id'].'. '.$comp['cat_name'] ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
		</div>
        ...
        <div class="row">
			<div class="col-md-6 form-group">
                Payee Name<select id="payee_name" name="payee_name" class="from-control form-control-sm select2" data-width="auto" required>
                    ...
                    ?>
                    <option value="<?= $row['id'] ?>" <?= isset($payee_name) && $payee_name == $row['id'] ? "selected" : "" ?>><?= $row['payee_name'] ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
			...
			<div class="col-md-2 form-group">
                VAT Deduction<input style="background-color: #f0dcf7;" type="number" id="vat_deduction" name="vat_deduction" class="form-control form-control-sm rounded-0" value="<?= isset($vat_deduction) ? $vat_deduction : "" ?>" onkeyup="net_pay()" required>
            </div>
			<div class="col-md-2 form-group">
                IT Deduction<input style="background-color: #f0dcf7;" type="number" id="it_deduction" name="it_deduction" class="form-control form-control-sm rounded-0" value="<?= isset($it_deduction) ? $it_deduction : "" ?>" onkeyup="net_pay()" required>
            </div>
        </div>
		<div class="row">
			<div class="col-md-2 form-group">
                Service Charge Deduction<input style="background-color: #f0dcf7;" type="number" id="sc_deduction" name="sc_deduction" class="form-control form-control-sm rounded-0" value="<?= isset($sc_deduction) ? $sc_deduction : "" ?>" onkeyup="net_pay()" required>
            </div>
			<div class="col-md-2 form-group">
                Sec Dep Deduction<input style="background-color: #f0dcf7;" type="number" id="security_deduction" name="security_deduction" class="form-control form-control-sm rounded-0" value="<?= isset($security_deduction) ? $security_deduction : "" ?>" onkeyup="net_pay()" required>
            </div>
			<div class="col-md-2 form-group">
                Cheque/Advice No.<input style="background-color: #f0dcf7;" type="text" id="chq_number" name="chq_number" class="form-control form-control-sm rounded-0" value="<?= isset($chq_number) ? $chq_number : "" ?>" required>
            </div>
			<div class="col-md-2 form-group">
			...
			<div class="col-md-2 form-group">
				Expenditure Type<br>
				<select id="exp_type" name="exp_type" class="from-control form-control-sm select2" data-width="auto" required>
					<option value="" disabled selected></option>
                    <option value="1" <?= isset($exp_type) && $exp_type == 1 ? "selected" : "" ?>>Prior Review</option>
					<option value="2" <?= isset($exp_type) && $exp_type == 2 ? "selected" : "" ?>>Post Review</option>
                </select>
            </div>
			<div class="col-md-2 form-group">
                Expenditure Group<select id="exp_group" name="exp_group" class="from-control form-control-sm select2" data-width="auto" required>
                    <option value="" disabled selected></option>
                    <?php 
                    $accounts = $conn->query("SELECT * FROM `exp_group`");
                    while($row = $accounts->fetch_assoc()):
                    ?>
                    <option value="<?= $row['id'] ?>" <?= isset($exp_group) && $exp_group == $row['id'] ? "selected" : "" ?>><?= $row['exp_name'] ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-5">
                Account-Economic Code (Dr)<select id="account_id" class="from-control form-control-sm select2" data-width="auto">
                    <option value="" disabled selected></option>
                    <?php 
            ...
            <div class="form-group col-md-2">
                Bank/Cash (Cr)<select id="gr_id" class="from-control form-control-sm form-control-border select2">
                    <option value="" disabled selected></option>
                    <?php 
...
function updateDliType() {
      const source_fund = document.getElementById('source_fund');
      const dli_type = document.getElementById('dli_type');
      const selectedFundSource = source_fund.value;

      // Clear existing options
      dli_type.innerHTML = '';

      // Populate child dropdown with options based on the selected country
      dlitypes[selectedFundSource].forEach(dlitype => {
        const option = document.createElement('option');
        option.value = dlitype;
        option.textContent = dlitype;
        dli_type.appendChild(option);
      });
    }
document.getElementById('source_fund').addEventListener('change', updateDliType);
updateDliType();
...
*/
	...
	var debit_count = $('#debit_count').val();
    ...
    $(function(){
        ...
        $('#add_to_list').click(function(){
			...
			tr.find('.delete-row').click(function(){
					var acc = $(this).closest('tr').find('.account').text()
					if (acc != "") {
						debit_count = debit_count - 1;
						$('#debit_count').val(debit_count);
						//alert(debit_count);
					}
					var gr = $(this).closest('tr').find('.group').text()
					if (gr != "") {
						credit_count = credit_count - 1;
						$('#credit_count').val(credit_count);
						//alert(credit_count);
        ...
        $('#uni_modal #journal-form').submit(function(e){
			...
			var pkg_number = $('#pkg_number').val()
			var category_number = $('#category_number').val()
			var component_number = $('#component_number').val()
			...
			if (exp_group == null){
					alert("Please select expenditure group!");
					return false;
				}
			if (pkg_number == null){
			...
			if (category_number == null){
					alert("Please select category number!");
					return false;
				}
			if (component_number == null){
					alert("Please select component number!");
					return false;
				}
