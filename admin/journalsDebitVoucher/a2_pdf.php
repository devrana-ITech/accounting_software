...
$journals = $conn->query("SELECT p.payee_name as p_name, p.contract_value, p.contract_value_us, pk.pack_name, p.contract_date, p.selection_method, p.contract_currency, je.component_number, p.percent_bank, p.id as pid, sum(je.gross_amt) as Crt from `journal_entries` je inner join payee p on p.id = je.payee_name inner join pkg pk on pk.id = p.pack_number WHERE journal_date >= '$start' and journal_date <= '$end' and dli_type = '$d_type' and je.exp_group = '$exp_group' and je.exp_type=1 group by p.payee_name, p.contract_value, p.contract_value_us, pk.pack_name, p.contract_date, p.selection_method, p.contract_currency, je.component_number, p.percent_bank, p.id;");
...
if ($journals->num_rows > 0){
	...
	while($row = $journals->fetch_assoc()){
		$payee_id = $row['pid'];
		$journals_details = $conn->query("SELECT je.journal_date, je.chq_number as chq_number from `journal_entries` je inner join payee p on p.id = je.payee_name WHERE p.iufr_flag = 1 and journal_date >= '$start' and journal_date <= '$end' and dli_type = '$d_type' and je.exp_group = '$exp_group' and je.exp_type=1 and p.id = '$payee_id';");
		...
		if ($journals_details->num_rows > 0){
			...
			if ($exp_group > 2){
				$journals_total = $conn->query("SELECT sum(je.gross_amt) as Crt_total from `journal_entries` je inner join payee p on p.id = je.payee_name WHERE journal_date <= '$end' and dli_type = '$d_type' and je.exp_group = '$exp_group' and je.exp_type=1 and p.iufr_flag = 1 and p.id = '$payee_id' group by p.id;");
				...
				if (($exp_group == 2) && ($d_type == 'Non-DLI')) {
					...
					$data .= '<td class="cl11" style ="padding: 3px; text-align: right;">'.number_format($row['Crt']*($row['percent_bank']/100), 2, '.', ',').'</td></tr>';
				}
				if (($exp_group == 2) && ($d_type == 'DLI')) {
					$data .= '<td class="cl10" style ="padding: 3px; text-align: center;">36.37%</td>';
					$data .= '<td class="cl11" style ="padding: 3px; text-align: right;">'.number_format($row['Crt']*(0.3637), 2, '.', ',').'</td></tr>';
				}
		  }
		}
...
<htmlpageheader name="letterheader">
	<div style="font-size: 10pt; text-align: center; padding-top: 1mm; font-family: nikosh; ">
		...
		<h4 style="margin-top: -5px; margin-bottom: -1px;">2A. Designated Account Expenditure for Contracts/Subject to Prior Review (Summary)</h4>
