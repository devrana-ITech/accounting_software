...
if ($journals->num_rows > 0){
	while($row = $journals->fetch_assoc()){
	   $data .= '<tr><td class="cl1" style ="padding: 3px;">'.$row['categoryname'].'</td>';
	   $data .= '<td class="cl2" style="padding: 3px; text-align: right;">'.number_format($row['fund_amt'], 2, '.', ',').'</td>';
	   $c1_total =  $c1_total + $row['fund_amt'];
	   $cat_id = $row['id'];
	   
	   if ($qtr_id == 3) {
		$journals1 = $conn->query("SELECT * from cashforecast WHERE qtr = '$qtr2' and year = '$year2' and fund_type = '$d_type' and category = '$cat_id';");
	   } else {
		$journals1 = $conn->query("SELECT * from cashforecast WHERE qtr = '$qtr2' and year = '$year' and fund_type = '$d_type' and category = '$cat_id';");
	   }
	   if ($journals1->num_rows == 1){
		while($row1 = $journals1->fetch_assoc()){
			$data .= '<td class="cl2" style="padding: 3px; text-align: right;">'.number_format($row1['fund_amt'], 2, '.', ',').'</td>';
			$c2_total =  $c2_total + $row1['fund_amt'];
			$r1_total =  $row1['fund_amt'] + $row['fund_amt'];
...
$data .= '<td class="cl2" style="padding: 3px; text-align: right;"><b>'.number_format($c2_total, 2, '.', ',').'</b></td>';
