...
$details = $conn->query("SELECT * FROM `journal_entries` where journal_date >= '$start' and journal_date <= '$end' and journal_type='dv' and dli_type = 'DLI' order by id asc");
...
while($row_d = $details->fetch_assoc()){
	...
	$data .= '<tr>'
	.'<td class="cl4">'.$sn.'</td>'
	.'<td class="cl1">'.$row_d['voucher_number'].' -- '.$row_d['description'].'</td>'
