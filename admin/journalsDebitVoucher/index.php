...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">Debit Voucher Entries</h3>
		<div class="card-tools">
			<button class="btn btn-primary btn-flat btn-sm" id="create_new" type="button"><i class="fa fa-pen-square"></i> Debit Voucher Entry</button>
		</div>
	</div>
	<div class="card-body">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<thead>
					<tr>
						...
						<th style="background: #faefe3; text-align: center;">Gross Amt etc.</th>
				...
				<tbody>
					<?php 
					$swhere = ""; //userdata('type') != 1 order by date(journal_date) LIMIT 100
					if($_settings->userdata('type') == 1){
						$swhere = " where user_id = '{$_settings->userdata('id')}' and year_id = '{$_settings->userdata('year_id')}' and journal_type='dv' ";
					}
				...
				//	$user_arr = array_column($users->fetch_all(MYSQLI_ASSOC),'username','id');
					...
					$journals = $conn->query("SELECT * FROM `journal_entries` {$swhere} order by voucher_number asc");
					...
					<tr>
						...
						<?php
						  ...
						  if ((!empty($row['vat_deduction'])) && ($row['vat_deduction'] != 0)){
							  $vat = "VAT: " .format_num($row['vat_deduction']). "<br>";
						  }
						  if ((!empty($row['it_deduction'])) && ($row['it_deduction'] != 0.00)){
							  $it = "IT: " .format_num($row['it_deduction']). "<br>";
						  }
						  if ((!empty($row['sc_deduction'])) && ($row['sc_deduction'] != 0)){
							  $sc = "Service: " .format_num($row['sc_deduction']). "<br>";
						  }
						...
						<td class="p-0">
							...
							<?php
								if ($p_id != 0)
								{
									$p_name = $conn->query("SELECT payee_name FROM `payee` where id = '$p_id'");
									while($pname = $p_name->fetch_assoc())
									{
										$payee_name = $pname['payee_name'];
									}
								}
							?>
							<div class="d-flex w-100">
								<div class="col-12 border"><i><?= $row['description'] ?></i>, <i><?= $payee_name ?></i></div>
...
<script>
	$(document).ready(function(){
		$('#create_new').click(function(){
			uni_modal("New Debit Voucher Entry","journalsDebitVoucher/manage_journal.php",'large')
		})
